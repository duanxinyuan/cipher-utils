...
public class DESUtils {
    ...
    public static byte[] encrypt(byte[] data, byte[] key, String iv, Mode mode, Padding padding) {
        ...
        try {
            ...
            String algorithm = AlgorithmUtils.getAlgorithm(Algorithm.DES, mode, padding);
            // 创建密码器
    ...
    public static byte[] genarateKey() {
        try {
            ...
            throw new RuntimeException("DES key genarate error", e);
        }
    }
    ...
    private static void check(byte[] key, String iv, Mode mode, Padding padding) {
        ...
        if (StringUtils.isNotEmpty(iv)) {
            checkIv(iv);
            if (mode == Mode.ECB) {
                throw new RuntimeException("DES ECB mode does not use an IV");
            }
        }
    }
    ...
    private static void checkModeAndPadding(Mode mode, Padding padding) {
        if (mode == Mode.NONE) {
            throw new RuntimeException("invalid DES mode");
        }
        if (padding == Padding.SSL3Padding || padding == Padding.PKCS1Padding) {
            throw new RuntimeException("invalid DES padding");
        }
        if (padding == Padding.NoPadding) {
            if (mode == Mode.ECB || mode == Mode.CBC) {
                throw new RuntimeException("invalid DES algorithm");
            }
        }
    }
    ...
    private static void checkKey(byte[] key) {
        if (key == null) {
            throw new RuntimeException("DES key cannot be null");
        }
        if (key.length != 8) {
            throw new RuntimeException("DES key not 8 bytes long");
        }
    }
    ...
    private static void checkIv(String iv) {
        if (iv.length() != 8) {
            throw new RuntimeException("DES iv not 8 bytes long");
        }
    }
